...
def get_s3_config():
    ...
    input_key = os.environ.get("S3_FILE_KEY")
    output_key = os.environ.get("S3_OUTPUT_KEY", "netflix_titles_limpio.csv")

    if not bucket:
        raise ValueError("No se encontró S3_BUCKET_NAME")
    if not input_key:
        raise ValueError("No se encontró S3_FILE_KEY")

    return bucket, input_key, output_key


@st.cache_data
def load_data() -> pd.DataFrame:
    bucket, input_key, _ = get_s3_config()
    s3 = get_s3_client()
...
def save_clean_data_to_s3(df: pd.DataFrame) -> str:
    bucket, _, output_key = get_s3_config()
    s3 = get_s3_client()
...
st.write(
    "Esta aplicación carga el dataset desde S3, revisa su calidad, limpia los datos "
    "y guarda una versión limpia en el mismo bucket."
)

try:
    bucket_name, input_file, output_file = get_s3_config()
    df_original = load_data()
except Exception as error:
    st.error(f"Ocurrió un problema al cargar el CSV desde S3: {error}")
    st.stop()

df_limpio, resumen_limpieza = clean_netflix_data(df_original)

try:
    archivo_generado = save_clean_data_to_s3(df_limpio)
    guardado_ok = True
except Exception as error:
...
with st.sidebar:
    ...
    st.write("Archivo leído desde S3:")
    st.code(input_file)
    st.write("Bucket:")
    st.code(bucket_name)
...
if guardado_ok:
    st.success(f"CSV limpio guardado en S3 como: {archivo_generado}")
else:
    st.warning(f"El CSV se limpió, pero no se pudo guardar en S3: {error_guardado}")
...
st.write(
    ...
    "Aquí se revisan duplicados, fechas, años fuera de rango, valores nulos "
...
st.write(
    "La limpieza se hizo conservando la mayor cantidad posible de información. "
    "Los campos faltantes como director, elenco, país o clasificación se marcaron "
...
st.write("4. Los faltantes categóricos se reemplazaron por etiquetas claras.")
...
st.write(
    "Las variables nuevas convierten columnas de texto en información más fácil de analizar. "
    "Por ejemplo, main_country toma el primer país listado y main_genre toma el primer género."
)
...
st.write(
    "En esta práctica se realizó un flujo completo de manejo de datos: carga desde S3, "
    "revisión inicial, auditoría, limpieza, creación de variables, visualización, "
    "descarga y guardado del resultado limpio en el bucket."
)
